...
class VendorController extends Controller
{
	         ...
	         return Datatables::of($datas)
                ...
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 1]).'" '.$ns.'>Deactivated</option></select></div>';
	                            ...
	                            ->addColumn('action', function(User $data) {
                                    ...
                                    <a href="' . route('admin-vendor-secret',$data->id) . '" > 
                                    <i class="fas fa-user"></i> Secret Login</a>

                                    <a href="' . route('admin-vendor-featured',$data->id) . '"> <i class="fas fa-edit"></i> Change featured status. Right now ('.$data->featured_vendor.')</a>
                                    
                                    <a href="javascript:;" data-href="' . route('admin-vendor-verify',$data->id) . '" class="verify" data-toggle="modal" data-target="#verify-modal"> <i class="fas fa-question"></i> Ask For Verification</a>
                                    
                                    <a href="' . route('admin-vendor-show',$data->id) . '" > <i class="fas fa-eye"></i> Details</a>
                                    
                                    <a data-href="' . route('admin-vendor-edit',$data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i> Edit</a>
                                    
                                
                                    
                                    <a href="javascript:;" class="send" data-email="'. $data->email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> Send Email</a><a href="javascript:;" data-href="' . route('admin-vendor-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a></div></div>';
	                            }) 
    ...
    public function featured($id)
    {
        ...
        if($data->featured_vendor=='unfeatured')
            DB::table('users')->where('id',$id)->update(['featured_vendor'=>'featured']);
        elseif($data->featured_vendor=='featured') DB::table('users')->where('id',$id)->update(['featured_vendor'=>'unfeatured']);
        else DB::table('users')->where('id',$id)->update(['featured_vendor'=>'unfeatured']);
        
        return back()->with('success','featured status changed');
    }
