...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $commission = Order::where('status','=','completed')->sum('pay_amount');
  

        $declined = Order::where('status','=','declined')->get();
        ...
        $browsers = Counter::where('type','browser')->orderBy('total_count','desc')->take(5)->get();
         ...
         $vp = VendorOrder::sum('price') -VendorOrder::sum('vorder_tax');

         $vp_commission = VendorOrder::where('status','completed')->sum('vorder_commission');

        $total_withdraw = round(Withdraw::sum('amount'),2);

        $total_completed_sell = round(Order::where('status','=','completed')->sum('pay_amount'),2);
        $total_completed_shipping= round(Order::where('status','=','completed')->sum('shipping_cost'),2);
        $commission=   round( $vp_commission);
        $commission_with_shipping =  round($commission +   $total_completed_shipping,2);

        

  
        $total_sell = round(Order::sum('pay_amount'),2);

       
        $total_earning =round( $total_sell - $vp,2);
        // dump(round($total_earning));die();

        $activation_notify = "";
        if (file_exists(public_path().'/rooted.txt')){
            // echo public_path().'/rooted.txt';die();
        ...
        return view('admin.dashboard',compact('pending','activation_notify','processing','completed','products','users','blogs','days','sales','pproducts','rorders','poproducts','rusers','referrals','browsers','declined','commission','total_sell','total_completed_sell','total_completed_shipping','commission_with_shipping','total_withdraw'));
    }
