...
class OrderController extends Controller
{
        ...
        //--- Redirect Section Ends     
            ...
            if ($input['status'] == "completed"){
                ...
                foreach($data->vendororders as $vorder)
                {
                    ...
                    // return response()->json($data->vendororders);
                    $uprice = User::findOrFail($vorder->user_id);
                    $vorder->price = $vorder->price - $vorder->vorder_tax; /* remove tax from original price */
                    $c_price = $vorder->price - $vorder->vorder_commission; /* remove custom product commission to original price without tax */

                    $c_price = $vorder->price - round($c_price*$gs->percentage_commission / 100) - $gs->fixed_commission; /* remove dynamic commission to original price without tax */

                    $c_price =   $c_price + $vorder->vorder_tax ; /* Add tax to without commission price*/
                    $uprice->current_balance = $uprice->current_balance + $c_price ;
                    $uprice->total_commission = $uprice->total_commission + $vorder->vorder_commission ; /** custom commission record */

                

                

                    // return response()->json( $uprice->current_balance);    
                    $uprice->update();
                }
    ...
    public function show($id)
    {
        ...
        // if($order->cart == 'android data')
        //  $cart = '';
        // else 
        //  $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
        //  return view('admin.order.details',compact('order','cart'));
        $order = DB::table('orders')->where('id',$id)->orderBy('id','desc')->first();
        if($order->cart == 'android data')
           $cart = '';
          else 
        //   $cart = unserialize(bzdecompress(utf8_decode($order->cart)));
        $cart = unserialize(urldecode(utf8_decode($order->cart)));
           return view('admin.order.details',compact('order','cart'));
    }
