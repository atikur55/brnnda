...
class VendorController extends Controller
{
    ...
    public function index()
    {
        ...
        $p = new Product;
        $v =  new VendorOrder;
        $vtotal = $p::vendorConvertPrice( $v::where('user_id','=',$user->id)->where('status','=','completed')->sum('price') ) ;
        
        $vtax = $v->where('user_id',$user->id)->where('status','=','completed')->sum('vorder_tax');

        if($vtotal){
            $vtax = $v->where('user_id',$user->id)->where('status','=','completed')->sum('vorder_tax');
            $vtotal = $vtotal - $vtax;
            $vtotal = $vtotal-( (float)$vtotal * (float)$gs->percentage_commission/100)-$gs->fixed_commission;
            $vtotal = $vtotal+$vtax;
        }
    
        $vtotal = round($vtotal);
        // dump(round($vtotal));die(); 
        ...
        return view('vendor.index',compact('user','pending','processing','completed','vtax','vtotal'));
    }
