...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        // if($tpr->product_tax!=0 or $tpr->product_tax!=null){
        //     if($tpr->product_tax_status==1){
        //         $tpr->price = $tpr->price + ($tpr->price*$tpr->product_tax/100);
        //     }
        ...
        //    $t = new Product;
        //    $tpr = $t->where('id',$tp['item']['id'])->first();
        //             if($tpr->product_tax!=0 or $tpr->product_tax!=null){
        //             if($tpr->product_tax_status==1){
                        
        //                 $products[$tp['item']['id']]['price'] = $tpr->price + ($tpr->price*$tpr->product_tax/100);
   ...
   public function addcart($id)
    ...
    public function walletcheck(){
        ...
        $w = $_GET['w'];
        $total = $_GET['total'];
        $fnd = User::where('id','=',$uid)->first();
      
       
        // $chkw = (float)$fnd->wallet*(50/100);
                
           
        $chkw = (float)$total*(50/100);

        if((float)$fnd->wallet>$chkw){
            $halftotal =  $chkw;
            Session::put('used_wallet',$halftotal);
            $data['halftotal'] = round($halftotal,2);
            $data['total'] = round($total,2);
            $data['new_wallet'] = (float)$fnd->wallet-$chkw ;
    
            return response()->json($data);  
    
        }else{
            $data['no_wallet'] = 'Sorry! Not enough amount in wallet';
            return response()->json(1);  
        }
    
    }

    public function wallet_un_check(){
        ...
        // if((float)$fnd->wallet>$chkw){
            // Session::put('wallet_amount', $w);
            // Session::put('stotal_amount', $total);
            // $halftotal =  $chkw;
            // $data['halftotal'] = round($halftotal,2);
            // $data['total'] = round($total,2);
            // $data['new_wallet'] = (float)$fnd->wallet+$chkw ;
    ...
    // }
        ...
        if(Session::has('coupon')){
            $total =$total+(float)Session::get('coupon');
        ...
        if($from <= $today && $to >= $today)
        {
            if($coupon->status == 1)
            {
                ...
                if($coupon->type == 0)
                {
                    ...
                    if(Session::has('used_wallet')){
                        $data[6] = Session::get('used_wallet');
                    }
