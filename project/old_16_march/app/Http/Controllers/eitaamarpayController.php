...
class eitaamarpayController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $post_data['amount'] = $request->total; # You cant not pay less than 10
...
// dump($post_data);die();
    ...
    //     dump($sd);
        return view('front.aamarpay',compact('request'));
    ...
    //     die();
        if ($order_detials->status == 'Pending') {
            // $validation = $sslc->orderValidate($tran_id, $amount, $currency, $request->all());
                // echo $validation;die();
            // if ($validation == TRUE) {
                if($request->pay_status == "Successful"){ /* Need to change after activated ssl */
...
// if($custom_request->pass_check) {
    ...
    if (!Session::has('cart')) {
        ...
        $item_number = 'BR'.time().str_random(2).'00'.$lastorder->id;
