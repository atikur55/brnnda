...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('cashback', function(Product $data) {
                                $sign = Currency::where('is_default','=',1)->first();
                                $cashback = round($data->cashback * $sign->value , 2);
                                $cashback = $sign->sign.$cashback ;
                                return  $cashback;
                            })
    ...
    public function store(Request $request)
    {
        ...
        $input['cashback'] = ($input['cashback'] / $sign->value);
        $input['commission'] = (float)$input['commission'] ;

        $input['product_tax'] = (float)$input['product_tax'] ;
        if(isset($request->product_tax_status)){
            $input['product_tax_status'] = $input['product_tax_status'] ;
         
         }else{
            $input['product_tax_status'] = null ;
         }
