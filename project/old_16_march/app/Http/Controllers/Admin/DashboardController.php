...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $total_withdraw = round(Withdraw::sum('amount'));

        $total_completed_sell = round(Order::where('status','=','completed')->sum('pay_amount'));
        $total_completed_shipping= round(Order::where('status','=','completed')->sum('shipping_cost'));
        $commission_with_shipping =  round($total_completed_sell - $total_withdraw);
        $commission=   round($total_completed_sell - $total_withdraw - $total_completed_shipping);
        $total_sell = round(Order::sum('pay_amount'));

        $vp = VendorOrder::sum('price') -VendorOrder::sum('vorder_tax');
        $total_earning =round( $total_sell - $vp);
