									...
									{{ Session::has('cart') ? count(Session::get('cart')->items) : '0' }}
													...
													@foreach(Session::get('cart')->items as $product)
														<li class="product cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
															...
															<figure class="product-image-container">
																...
																<div class="cart-remove" data-class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}" data-href="{{ route('product.cart.remove',$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values'])) }}" title="Remove Product" onclick="chkzero()">
														...
														<script>
														function chkzero(){
														    aaa = document.getElementById('cart-count');
														    if (aaa.innerHTML == '1'){
														        aaa.style.display = 'none'
														        console.log(aaa.innerHTML)
														    }
														    else if(aaa.innerHTML == '0'){
												...
												<div class="dropdown-cart-action">
														<a href="{{ route('front.cart') }}" class="mybtn1">View Cart</a>
												</div><!-- End .dropdown-cart-total -->
										</div>
									@else 
