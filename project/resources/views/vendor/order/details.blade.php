...
@section('content')
<?php   use App\Models\Product; $comcount = 0; ?>
...
$price = number_format($order->vendororders()->where('user_id','=',$user->id)->sum('price'),2);
  ...
  if($user->shipping_cost != 0){
      ...
      echo 'sp = '.$price;
    } /* this can cause prob if shipping cost is not null as price is string not float - Eit saad */

  if($order->tax != 0){
    $tax = ((float)$price / 100) * (float)$order->tax;
    $price  += (float)$tax;
    echo 'np = '.$price;
    }    /* this can cause prob if tax is not null */
    $price2 = $price;
    echo 'p = '.(float) $price2;
...
{{-- 
                                                    <td width="45%">{{$order->currency_sign}} {{ round((float)$price * (float)$order->currency_value , 2) }}</td> --}}
                                                ...
                                                @if($order->method != "Cash On Delivery")
                                                    ...
                                                    <tr>
                                                        <th width="45%">aamarpay transcation ID </th>
                                                        <td width="10%">:</td>
                                                        <td width="45%">{{$order->ssl_transaction_id }}</td>
                                                    </tr>
                                                    @endif
                                                @endif
                              ...
                              <a href="javascript:;" data-toggle="modal" data-target="#confirm-delete" class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i class="fa fa-eye"></i> View License</a>
                                            ...
                                            <td>
                                               ...
                                               @if($product['color'])
                                                ...
                                                @endif
                                                <p>
                                                        <strong>{{ $langg->lang754 }} :</strong> {{$order->currency_sign}}{{ round($product['item']['price'] * (float)$order->currency_value , 2) }}
                                                </p>
                                               <p>
                                            ...
                                            <td>{{$order->currency_sign}}{{ round((float)$product['price'] * (float)$order->currency_value , 2) }}</td>
                                            {{-- <td> --}}
                                               <?php 
                                                ...
                                                //  $getp = $fp->where('id',$product['item']->id )->first();
                                              
                                                //  $comcount += $getp->commission*$product['qty'];
                                            //    dump($getp->commission*$product['qty']);
                                               
                                               ?>

                                              {{-- {{$order->currency_sign}} {{$getp->commission*$product['qty']}} --}}
                                            {{-- </td> --}}
                ...
                @endif
                                @endforeach
                                                    ...
                                                    <h3>NET PRICE: {{ $price}}  {{$order->currency_sign}}</h3>
                                                    {{-- <h3>NET COMMISSION: {{ $comcount}}  {{$order->currency_sign}}</h3> --}}
                                            </div>
                                        </div>
                                    </div>
