											...
											<div class="row">
												...
												<div class="col-lg-6">
													<input class="form-control" type="text" name="phone"
														placeholder="{{ $langg->lang153 }}" readonly required=""
										...
										<div class="order-area">
											...
											<div class="order-item">
												...
												<div class="product-content">
													...
													<div class="total-price">
														<h5 class="label">Cashback : </h5>
														<?php 
															$d =  App\Models\Product::where('id',$product['item']['id'] )->first();
														?>
														<p>
															@isset( $d->cashback )
															{{$d->cashback }} 
															@else 0
															@endisset X {{ $product['qty'] }} = {{(float)$d->cashback*(int)$product['qty'] }}

															<?php $tcb = $tcb +(float)$d->cashback*(int)$product['qty']   ?>৳
														</p>
													</div>
												</div>
											</div>
												...
												<div class="row">
													<div class="col-lg-12">
														...
														@endif  
														 ...
														 <a class="nav-link payment" data-val="" data-show="no" data-form="{{url('/pay')}}" data-href="{{ route('front.load.payment',['slug1' => 'cod','slug2' => 0]) }}" id="v-pills-tab3-tab" data-toggle="pill" href="#v-pills-tab3" role="tab" aria-controls="v-pills-tab3" aria-selected="false">
{{-- 
																<a class="nav-link payment" data-val="" data-show="no" data-form="{{aamarpay_payment_url()}}" data-href="{{ route('front.load.payment',['slug1' => 'cod','slug2' => 0]) }}" id="v-pills-tab3-tab" data-toggle="pill" href="#v-pills-tab3" role="tab" aria-controls="v-pills-tab3" aria-selected="false">
											 --}}
...
@endif
	...
	<button type="submit" id="final-btn" id="sslczPayBtn"
		...
		class="mybtn1">{{ $langg->lang753 }}
							...
							@endif
							<input type="hidden" name="total_cashback" value="{{$tcb}}">
							...
							{!!
								aamarpay_hidden_input([
									...
									'tran_id'   =>rand(10000,99999).$d, // random number. if you don't set this it will be auto generate.
									'cus_name'  =>  Auth::check() ? Auth::user()->name : '' , // Customer name
									'cus_email' =>  Auth::check() ? Auth::user()->email : '' , // Customer email
									'cus_phone' =>  Auth::check() ? Auth::user()->phone : '' , // Customer Phone
									// 'amount' =>  round($totalPrice * $curr->value,2),
									'amount' => $totalPrice ,
									'currency' =>  "BDT",
							...
							<input type="hidden" name="left_wallet" id="input_used_wallet" >
...
{{--  							
						<div class="wallet">
						    ...
						    <form id="use-wallet-form" class="wallet"> 
						        <input type="text" value="{{(float)((float)Auth::user()->wallet*(float)(50/100))}}"  id="wallet" readonly>
						        <button type="submit">Use</button>
						    </form>


						</div>  --}}
	@isset(Auth::user()->wallet)
						<div class="wallet-area">
							<h4 class="title">Use Wallet</h4>

				
						
							<div class="wallet-design">
									<input type="radio" class="wallet1" id="wallet1" name="use_wallet" value="		@isset(Auth::user()->wallet) {{(float)Auth::user()->wallet}}   @endisset" > 
									<span class="checkmark"></span>
									<label > 
									Yes
								
									<small id="use_wallet_amount">({{Auth::user()->wallet}} ৳)</small>
											{{-- <small>50% of total wallet ({{(float)((float)Auth::user()->wallet*(float)(50/100))}} ৳)</small> --}}
									</label>
							
							</div>
							<div class="wallet-design">
								<input type="radio" class="wallet2" id="wallet2" name="use_wallet" value="2" checked> 
								<span class="checkmark"></span>
								<label > 
								No
...
/* add wallet start eit code */
var w ='';
$('.wallet1').on('click',function(){
    w = $(this).val();
	// var wttotal = parseFloat($('#tgrandtotal').val()) -parseFloat(w);
	// $('#final-cost').html(wttotal+'{{ $curr->sign }}');
	// $('#grandtotal').val(wttotal);

	var val = $("#code").val();
        var total = parseFloat($("#grandtotal").val()) - mship;
		@isset(Auth::user()->id) 		var uid =  {{Auth::user()->id}} 
        ...
        var ship = 0;
            $.ajax({
                    type: "GET",
                    url:mainurl+"/walletcheck",
                    data:{w:w, total:total,uid:uid},
                    success:function(data){
						var halftotal = data['halftotal'];
						var g_total=data['total'];
						var new_wallet=data['new_wallet'];
						console.log(data);
						console.log(data['no_wallet']);
                        if(data == 0)
                        {
                        	toastr.error("wallet removed");
                            $("#code").val("");
                        }
				
						else if(data==1){

							toastr.error("Sorry! Not enough amount in wallet");
							$("#code").val("");
							}
                        else
                        {
								$('#grandtotal').val(halftotal+mship);
								$('#tgrandtotal').val(halftotal+mship);
							


								// var ttotal = parseFloat($('#grandtotal').val()) +data[0];
								// ttotal = parseFloat(data[0]);
								var ttotal = parseFloat($('#grandtotal').val()) +halftotal+mship;
								ttotal = parseFloat(halftotal+mship);
									if(ttotal % 1 != 0)
									{
									...
									document.getElementById("wallet1").checked = true;
									document.getElementById("wallet2").checked = false;
							
									$('small#use_wallet_amount').text(new_wallet);
									$('#input_used_wallet').val(new_wallet);

									toastr.success("Wallet amount added");
        ...
        var ship = 0;
            $.ajax({
                    ...
                    success:function(data){
						...
						// 		$('#tgrandtotal').val(data['total']);
							
				

						// 		var ttotal = parseFloat($('#grandtotal').val()) +data['total'];
						// 		// ttotal = parseFloat(data[]);
						// 			if(ttotal % 1 != 0)
						// 			{
						// 				ttotal = ttotal.toFixed(2);
						// 			}
						...
						// 			$('#final-cost').html(ttotal+'{{ $curr->sign }}')
						// 			document.getElementById("wallet1").checked = false;
						// 			document.getElementById("wallet2").checked = true;
						// 			$('#input_used_wallet').val(0);
						// 			toastr.success("Wallet amount removed");
            ...
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        else
                        {
							...
							var mtotal_cost = data[0]-parseFloat(mship)+parseFloat(used_wallet);

							if(pos == 0){
								$('#total-cost').html('{{ $curr->sign }}'+mtotal_cost);
								$('#discount').html('{{ $curr->sign }}'+data[2]);
							}
							else{
								$('#total-cost').html(mtotal_cost+'{{ $curr->sign }}');
								$('#discount').html(data[2]+'{{ $curr->sign }}');
							}
								$('#grandtotal').val(mtotal_cost);
								$('#tgrandtotal').val(mtotal_cost);
				...
				var ttotal = parseFloat($('#grandtotal').val()) + parseFloat(mship) + parseFloat(mpack) - parseFloat(used_wallet);
				ttotal = parseFloat(ttotal);
					if(ttotal % 1 != 0)
					{
						ttotal = ttotal.toFixed(2);
...
// Step 2 btn DONE
	...
	$('#step3-btn').on('click',function(){
		...
		// $('#final-btn').attr('id', 'sslczPayBtn');
		// $('#sslczPayBtn').html("Online payment");
...
/* SSL COMMERZ */
    ...
    console.log('ss = ');
	var ss = $("#sslczPayBtn").attr("postdata");
	// console.log(ss);
